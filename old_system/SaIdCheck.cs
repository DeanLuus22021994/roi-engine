...
namespace DiaFaceApp
{
    class SaIdCheck
    {
        ...
        // constructor
        public void IDValidator(string id_)
        {
            id = id_;
        }
        ...
        public string GetDateOfBirth()
        {
            if (isValid())
            {
                DateTime date = DateTime.ParseExact(id.Substring(0, 2) + "/" + id.Substring(2, 2) + "/" + id.Substring(4, 2), "yy/MM/dd", Globals.cInfo);
                int years = DateTime.Now.Year - date.Year;

                if (years < 0)
                {
                     date = DateTime.ParseExact("19" + id.Substring(0, 2) + "/" + id.Substring(2, 2) + "/" + id.Substring(4, 2), "yyyy/MM/dd", Globals.cInfo);
                }
